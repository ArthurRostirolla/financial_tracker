...
export default function DespesaForm({ onSuccess, categorias, contas, despesaToEdit }) {
  ...
  useEffect(() => {
    if (despesaToEdit) {
      setForm({
        ...
        conta_id: despesaToEdit.conta_id || '',
        data_despesa: despesaToEdit.data_despesa ? new Date(despesaToEdit.data_despesa).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
      });
    }
  }, [despesaToEdit]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const url = despesaToEdit ? `/api/despesas?id=${despesaToEdit.id}` : '/api/despesas';
   ...
   return (
    <form onSubmit={handleSubmit} className="space-y-4 text-text-light dark:text-text-dark">
      <input name="descricao" value={form.descricao} placeholder="Descrição (ex: Almoço)" onChange={handleChange} required className="w-full p-2 border rounded bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600" />
      ...
      <input name="data_despesa" value={form.data_despesa} type="date" onChange={handleChange} required className="w-full p-2 border rounded bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600" />
      <button type="submit" className="w-full px-4 py-2 bg-sidebar-light text-white rounded-lg shadow hover:bg-opacity-90">
        {despesaToEdit ? 'Atualizar Despesa' : 'Adicionar Despesa'}
      </button>
    </form>
  );
}